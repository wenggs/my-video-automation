...
- Rules-based tags suggestion API (`POST /api/v1/library/videos/{id}/tags/suggest`, with reason details) + fine-grained accept/reject (`POST /tags/suggestions/accept|reject`)
...
- UI operations: tags load/suggest(save-by-chip)/save + recent-tags quick add + job-tag filter, auto-generate lyrics (quality/model/language), retry/cancel auto-generate, request_id display, minimal subtitle review/save, auto metrics display, create job, auto-refresh, status filter, error detail expand/copy, workspace config panel
